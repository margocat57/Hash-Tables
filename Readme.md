...
Хэш-таблица хранит ключи так, что под каждую строку выделяется 32 байта, так как в тестовом наборе максимальная длина слова равна 32 символам(с учетом \0), а более длинные слова в целом встречаются редко. Если строка короче 32 символов, оставшаяся часть заполняется нулевыми байтами. Такой формат упрощает дальнейшие оптимизации хэш таблиц.

#### Немного про линеаризацию

Также у каждого бакета есть флаг `is_linearized`, если он равен true, то применяются оптимизированные версии функции `find_node`. После вставки или удаления элемента он сбрасывается в false, потому что данные внутри бакета могли перестать быть удобными для оптимизированного поиска.
...
В основном использовлся флаг оптимизации -О3, в части тестов были использованы флаги -О2 и -О0
...
|-----|-----|-----|
| Без оптимизаций -О0 | 3.20 | 4.83 |
| Без оптимизаций -О2 | 4.33 | 4.31 |
| Без оптимизаций -О3 | 1.61 | 2.48 |
| Оптимизация 1 | 4.27 | 2.31 |
| Оптимизация 2 | 2.80 | 1.80 |
| Оптимизация 3 | 2.49 | 2.29 |
| PGO | 3.66 | 2.80 |
...
### Работа базовой версии хэш-таблицы с оптимизациями компилятора. Выбор флага оптимизацми

| Уровень оптимизации | Асимптотический доверительный интервал (секунд) |
| :--- | :--- |
| -О0 | 0.703 ± 0.001 |
| -О2 | 0.4127 ± 0.0003 |
| -О3 | 0.4095 ± 0.0006 |

Видим, что оптимизации компилятора стоит использовать: в нашем случае использование флага -О3 дало ускорение примерно в 1,717 раза.

Покажем, что существует статистически значимая разница между использованием уровня опатимизации -О2 и -О3.
Нулевая гипотеза - распределения времени выполнения с использованием -О2 и -О3 совпаают, то есть различия между измерениями отсутствуют. Уровень значимости принят равным $\alpha = 0.05$. Для проверки гипотезы используется U-критерий Манна–Уитни.

p-value=$1 \cdot 10^{-43}$, что говорит о наличие статистически значимых различий между данными уровнями оптимизации -О2 и -О3.

Вариант с -O3 показал наилучшее время работы и статистически значимые различия по сравнению с -О2. Поэтому дальнейшее профилирование проводилось именно при -O3, и оптимизации кода были направлены на устранение узких мест, обнаруженных с этим флагом компиляции. Все последующие версии также компилировались только с -O3.
